...
import { getSupplierList, createSupplier, updateSupplier, deleteSupplier } from '@/api/system'
import type { Supplier } from '@/api/types/system'
...
const columns: TableColumn[] = [
  { prop: 'code', label: '供应商编码', minWidth: '110' },
...
const handleEdit = (row: Supplier) => { isEdit.value = true; Object.assign(form, row); dialogVisible.value = true }
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (isEdit.value && form.id) { await updateSupplier(form.id, form); ElMessage.success('更新成功') }
...
const handleDelete = (row: Supplier) => {
  ElMessageBox.confirm(`确定删除供应商「${row.name}」吗？`, '提示', { type: 'warning' })
    .then(async () => { await deleteSupplier(row.id); ElMessage.success('删除成功'); getList() }).catch(() => {})
}
...
.supplier-page { .search-card { margin-bottom: 16px; } .table-header { margin-bottom: 16px; } }
</style>
