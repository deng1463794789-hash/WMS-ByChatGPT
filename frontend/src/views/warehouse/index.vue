<template>
  <div class="warehouse-page">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑仓库' : '新增仓库'" width="550px" destroy-on-close>
...
import { getWarehouseList, createWarehouse, updateWarehouse, deleteWarehouse } from '@/api/system'
import type { Warehouse } from '@/api/types/system'
...
const columns: TableColumn[] = [
  { prop: 'code', label: '仓库编码', minWidth: '110' },
  { prop: 'name', label: '仓库名称', minWidth: '130' },
  { prop: 'address', label: '地址', minWidth: '160', showOverflowTooltip: true },
...
const initialForm = { id: undefined as number | undefined, code: '', name: '', address: '', manager: '', phone: '', status: 'active' }
...
const handleEdit = (row: Warehouse) => { isEdit.value = true; Object.assign(form, row); dialogVisible.value = true }
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (isEdit.value && form.id) { await updateWarehouse(form.id, form); ElMessage.success('更新成功') }
      else { await createWarehouse(form); ElMessage.success('新增成功') }
...
const handleDelete = (row: Warehouse) => {
  ElMessageBox.confirm(`确定删除仓库「${row.name}」吗？`, '提示', { type: 'warning' })
    .then(async () => { await deleteWarehouse(row.id); ElMessage.success('删除成功'); getList() }).catch(() => {})
}
...
.warehouse-page { .search-card { margin-bottom: 16px; } .table-header { margin-bottom: 16px; } }
</style>
